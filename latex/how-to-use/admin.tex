...
\midrule
対象大会 & 結果入力前に、現在の大会が \code{active}（開催中）であり、当日運用している大会と一致しているか確認する。 \\
\midrule
...
なお、MyIDバーコード読み取りによる対象試合へのチェックイン（参加本登録を兼ねる）は、
必要に応じて実施する任意の操作である。
...
\node[opt, right=0.5cm of s3] (myid) {MyID読み取り（任意）→ 第\ref{sec:myid}章};
...
自分が担当するクラスの情報を確認する。
この画面はadminおよびroot専用であり、任意のクラスを対象にできる。
...
原則として、同じ学生は同一大会で1競技まで登録できる。rootが大会情報管理で設定した人数以下のクラスは、1人につき2競技まで登録できる。
専攻科・教員チーム（\code{専教}）は例外として重複登録の上限を設けない。
登録時に上限を超える学生が含まれている場合は、割り当て全体が拒否されるため、既存の参加競技を確認してから登録する。
重複登録が許可される場合も、当日の時間割と重複していないか確認する。

\subsection{盤上競技の代表者登録}

将棋では各クラスの代表A・代表B・補欠を、オセロでは代表者を登録する。
盤上競技の参加クラスは16クラス固定であり、rootのトーナメント生成画面では参加クラスを選択しない。
そのため、トーナメント生成前にこの画面で各クラスの代表者が揃っていることを確認する。

\subsection{チームメンバーの削除}

割り当て済みのメンバーをチームから外すことができる。
大会開始後の削除は、試合進行やMyIDチェックインに影響する可能性があるため、担当者間で確認してから行う。
...
競技参加用ロールは大会単位で保持される。現在の大会で付与したロールは、別大会の通知対象や競技割り当てには使用されない。

\subsection{ロールの削除}

付与済みのロールを削除する。
クラス単位の操作対象はロールではなく、プロフィールの所属クラスで決まる。

\newpage

% =====================================================================
\section{MyIDバーコード読み取り・チェックイン}\label{sec:myid}
% =====================================================================
...
学生証等のMyIDバーコードを読み取り、選択した通常競技または昼競技の試合へのチェックインを記録する機能である。読み取り時に参加本登録も行う。
MyIDは \code{H10} に7桁の学籍番号が続く形式（例: \code{H102301059}）として扱われる。
読み取った学籍番号は、\code{s<学籍番号>@sendai-nct.jp} 形式のメールアドレスを持つユーザーと照合される。

\begin{enumerate}
  \item 「競技」を選択する。昼競技では、対象セッションに連携した競技を選択する。
  \item 選択した競技の試合一覧から、受付対象の「試合」を選択する。通常競技では選択した試合のラウンド、昼競技では選択した昼競技試合へチェックインされる。
  \item 「読み取り開始」を押し、カメラで学生のMyIDバーコードを読み取る。
  \item カメラが使えない場合は、「手入力」にMyID値を入力して「チェックインする」を押す。
  \item 読み取りに成功すると、参加本登録と対象試合へのチェックインが完了する。
\end{enumerate}

選択した試合が対象イベント・対象競技に存在しない場合は登録できない。
通常競技では、選択した試合の対戦チームに割り当て済みのメンバーだけが対象となる。昼競技では、試合対象クラスのうち、当該昼競技に割り当て済みのメンバーだけが対象となる。クラスの全在籍者は対象にならない。
通常競技のラウンド番号は手入力せず、選択した試合のトーナメント情報から自動的に決定される。通常競技では事前にトーナメントを生成しておく。昼競技では対象セッションと試合を作成しておく。
...
\subsection{チェックイン状況の確認}

チェックイン状況は、同じバーコード読み取り画面で確認する。選択した試合について、対象クラスごとに「チェックイン済み人数 / 対象人数」とチェックイン済みメンバーの表示名を表示する。
対象人数から表示名を照合することで、未チェックイン者を確認できる。通常競技・昼競技ともにこの画面で確認する。
従来の \code{/dashboard/admin/confirmed-participants}（参加本登録済みメンバー確認）画面は廃止した。
...
\subsection{競技概要・ルールPDFの登録}

競技の説明を記述し、PDFファイルの競技要項をアップロードする。
競技概要とルールPDFは学生の競技情報画面にも表示される。
...
結果入力は、大会ステータスが \code{active}（開催中）の場合のみ可能である。\code{preparing}（準備中）、\code{upcoming}（予定）、\code{archived}（アーカイブ）では入力できない。
...
結果入力は、大会ステータスが \code{active}（開催中）の場合のみ可能である。
...
テンプレートランの結果入力も、大会ステータスが \code{active}（開催中）の場合のみ可能である。

\begin{itemize}
  \item \textbf{年リレー} ── ブロックごとの結果と総合ボーナス
  \item \textbf{コースリレー} ── コース別の結果
  \item \textbf{綱引き} ── 各試合の勝敗
  \item \textbf{借り物競争} ── 順位と競技内スコア
\end{itemize}

テンプレートランはrootが事前に生成した対戦カードに対して結果を入力する。
ドラッグ操作で順位を並べる形式の入力がある場合は、保存前に順位順が正しいか確認する。

\subsection{競技タイピング結果のインポート}

競技タイピングでは、独自タイピングシステムが出力した \code{typing-results-v1} 形式のJSONを選択し、
「結果をインポート」で順位と大会得点を確定する。
同一結果を再度選んだ場合は取込済みとして扱われる。訂正済みファイルへ差し替える場合だけ
「置換インポート」を使用し、確定後に順位点と学生画面の表示を確認する。
...
\midrule
チームメンバー割り当て & 通常競技の参加者と、将棋・オセロの代表者を登録・削除 \\
\midrule
ロール付与・削除 & 大会単位の審判・競技担当など任意ロールを付与または削除 \\
\midrule
MyIDバーコード読み取り & 学生のMyIDバーコードを読み取り、通常競技・昼競技の対象試合へチェックイン（参加本登録を兼ねる） \\
\midrule
チェックイン状況の確認 & 試合・クラスごとのチェックイン済み人数 / 対象人数、表示名、未チェックイン者を確認 \\
\midrule
...
\midrule
昼競技結果の入力 & 各テンプレートの結果を登録し、競技タイピングのJSONを取り込む \\
\midrule
