...
\begin{itemize}
  ...
  \item \textbf{admin} ── 本書が対象とする権限。試合進行・結果入力など大会運営補助が可能。
...
\node[main, below of=s1] (s2) {② クラス代表ロール付与（事前準備）　→ 第\ref{sec:role}章};
...
クラス代表（\code{クラス名\_rep} ロールを持つユーザー）も同様の操作が可能である。

\subsection{担当クラスの確認}

自分が担当するクラスの情報を確認する。
ロール管理でクラス代表ロールが割り当てられたクラスが対象となる。
...
学生に対してクラス代表ロールを付与・削除する機能である。

\subsection{クラス代表ロールの付与}

対象ユーザーを検索し、\code{クラス名\_rep} 形式のクラス代表ロールを付与する。
クラス代表ロールを持つ学生は、自クラスのチーム管理操作が可能になる。

\subsection{クラス代表ロールの削除}
...
MIC（Most Improved Class：最も改善したクラス）の投票を行う機能である。
...
\midrule
クラス代表ロール付与・削除 & 学生に \code{クラス名\_rep} ロールを付与または削除 \\
\midrule
...
\midrule
MIC投票 & Most Improved Classの候補に投票 \\
\bottomrule
\end{longtable}
