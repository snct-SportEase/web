...
初回ログイン時に選択したクラスはプロフィールに保存される。
クラス情報やクラス単位の操作は、この所属クラスを対象として行われる。プロフィール完了後は一般ユーザーによる所属クラス変更はできない。
...
\node[main, below of=s2] (s3) {③ トーナメント・結果一覧・スコア閲覧　→ 第\ref{sec:tournament}章};
\node[main, below of=s3] (s4) {④ 通知受信・管理　→ 第\ref{sec:notification}章};

\node[opt, right=0.5cm of s2] (myid) {MyID提示（必要時）→ 第\ref{sec:myid}章};
\node[opt, right=0.5cm of s4] (req) {通知申請（任意）→ 第\ref{sec:notificationreq}章};
...
自分のプロフィール、クラス成績、参加試合、試合結果、クラス状況、最新のお知らせ、
大会資料を一画面で確認できるページである。
...
rootの設定により一般学生へのスコア公開が停止されている場合は、順位や得点を表示しない。
その場合も、参加試合、試合結果、クラス状況、お知らせ、大会資料は引き続き確認できる。

\subsection{参加試合}

自分が割り当てられている競技と、今後の通常競技・昼競技の日時、種目、対戦相手、場所を確認できる。
集合前にこの欄を確認し、試合開始時刻に遅れないように移動する。
組み合わせや開始時刻が未確定の場合は「未定」と表示されることがある。

\subsection{試合結果}

自分が所属するチームの通常競技と、自クラスが参加した昼競技について、
直近5試合の勝敗、スコア、ラウンドを確認できる。

\subsection{クラス状況}

自クラスの在籍人数と出席人数、参加競技数、競技ごとのチーム名・現在ラウンド・進捗状況を確認できる。
直近の試合結果が登録されている競技では、その結果も表示する。

\subsection{最新のお知らせ}

運営から配信された最新3件のお知らせについて、件名、配信日時、本文の概要を確認できる。
すべてのお知らせを確認する場合は、通知一覧画面を使用する。

\subsection{大会資料・リンク}

運営が登録した大会要項PDFと競技別の競技要項PDFを、マイページから別タブで開くことができる。
大会終了後などにrootがアンケートを公開した場合は、「アンケートに回答する」リンクも表示される。
未登録のPDFや未公開のアンケートは表示されない。
...
大会の通常競技と盤上競技（将棋・オセロ）のトーナメントブラケットを確認するページである。
...
将棋は代表A・代表Bの2ブラケット、オセロはクラス対抗のブラケットとして表示される。
出場者名だけの一覧ではなく、対戦枠と勝ち上がりをブラケット上で確認する。
...
% =====================================================================
\section{結果一覧}\label{sec:results}
% =====================================================================

\textbf{ページ}: \code{/dashboard/student/results}

開催中大会の確定済み結果を、競技を横断して確認するページである。

\subsection{通常競技の結果}

完了した試合について、競技名、ラウンド（3位決定戦を含む）、対戦チーム、
スコア、勝者を表示する。結果が確定していない試合は一覧に含まれない。

\subsection{昼競技の結果}

公開済みの各昼競技セッションについて、順位と獲得点、または勝者を表示する。
競技タイピングなど順位形式の競技も、運営が結果を確定した後に表示される。
...
運営スタッフはMyIDバーコードを読み取り、対象試合へのチェックイン（参加本登録を兼ねる）を行う。
...
\begin{enumerate}
  ...
  \item 読み取りが完了すると、参加本登録および対象試合へのチェックインが行われる。
\end{enumerate}

\subsection{注意事項}

MyIDバーコードが読み取れない場合は、運営スタッフに学籍番号を伝えて手入力で確認してもらう。
ただし、対象試合に割り当てられていない場合はチェックインできない。
...
プロフィールでクラスを設定した学生が、自分のクラスについてアクセスできるページである。
...
\midrule
\endhead
マイページの閲覧 & クラス成績、参加試合、直近の結果、クラス状況、お知らせ、大会資料・アンケートを確認 \\
\midrule
トーナメントの閲覧 & 競技ごとのトーナメントブラケットと試合結果を確認 \\
\midrule
結果一覧の閲覧 & 完了した通常試合と公開済み昼競技結果を競技横断で確認 \\
\midrule
...
\midrule
MyIDによる参加確認 & 大会当日にMyIDバーコードを提示し、運営スタッフによる対象試合のチェックインを受ける \\
\midrule
