...
\midrule
ステータス & 試合運用を開始する大会だけを \code{active} にする。同時にactiveにできる大会は1つのみである。 \\
\midrule
...
\code{active} にした大会は、学生・adminの各画面で「現在の大会」として扱われる。
大会準備中は \code{upcoming} のまま設定を行い、当日の運用開始直前に \code{active} へ切り替える。
