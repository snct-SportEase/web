...
\begin{itemize}
  ...
  \item \textbf{student} ── エントリー・観戦・通知受信のみ。
\end{itemize}
...
なお、ホワイトリスト設定（⑦）を完了するまでは admin 以下の権限を持つユーザーはシステムにアクセスできない。
すべての初期設定を root 単独で完了させてから、他権限ユーザーのアクセスを解放する運用を推奨する。
...
\node[main, below of=s6] (s7) {⑦ ホワイトリスト設定（admin以下アクセス解放）　→ 第\ref{sec:whitelist}章};
...
大会に設定したアンケートURLを、システム全体の通知として一斉配信できる。
配信後、学生のダッシュボードに通知として表示される。

\subsection{得点CSVインポート（春季限定）}
...
各クラスの登録学生数を設定する。
この数値はトーナメント生成時のシード計算や定員判定に使用される。
...
\code{class\_id,student\_count} 形式のCSVをアップロードすることで、
全クラスの人数を一度に更新できる。
...
\begin{itemize}
  \item \textbf{開催場所} ── gym1 / gym2 / ground / other から選択
  \item \textbf{競技概要・ルール} ── Markdown形式で詳細を記述
  \item \textbf{最低定員・最高定員} ── 1チームあたりの参加人数の範囲
\end{itemize}
...
% =====================================================================
\section{競技要項のアップロード}\label{sec:guidelines}
% =====================================================================

\textbf{ページ}: \code{/dashboard/root/competition-guidelines-upload}

大会の競技要項をPDFファイルとしてアップロードし、学生が参照できる状態にする。
...
% =====================================================================
\section{ホワイトリスト管理}\label{sec:whitelist}
% =====================================================================

\textbf{ページ}: \code{/dashboard/root/whitelist-management}

システムへのログインを許可するメールアドレスの一覧（ホワイトリスト）を管理する。
ホワイトリストに登録されていないメールアドレスはログインできない。

\subsection{メールアドレスの追加}

メールアドレスと付与するロール（student / admin / root）を指定して追加する。

\subsection{CSVによる一括登録}

\code{email,role} 形式のCSVファイルをアップロードすることで、複数のメールアドレスを一括登録できる。

\subsection{メールアドレスの削除}

単一のメールアドレスを個別削除するほか、複数を選択して一括削除することもできる。

\subsection{検索・フィルタリング}

登録件数が多い場合に備えて、メールアドレスの検索・フィルタリング・ソートが利用できる。
...
\subsection{ロールの付与・削除}

ユーザーに割り当てられているロール（student / admin / root）を追加または削除できる。

\subsection{クラス代表ロールの付け替え}

\code{クラス名\_rep} 形式のクラス代表ロールを変更する際、
旧ロールの削除と新ロールの付与が自動的に行われる。
これにより、一人のユーザーが複数クラスの代表になる状況を防ぐ。
...
MIC（Most Improved Class：最も改善したクラス）の確認ページである。
...
\midrule
ホワイトリスト管理 & ログイン許可メールの追加・削除・CSV一括登録 \\
\midrule
...
\midrule
クラス代表ロール付け替え & クラス代表ロールの変更（旧ロール自動削除） \\
\midrule
