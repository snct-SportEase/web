...
本システムでは、ユーザー認証にGoogle OAuth2を採用している。学校のGoogle アカウント(@sendai-nct.jp, @sendai-nct.ac.jp)のみのログインを許容するためホワイトリストを作成し、制御をしている。
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 卒業生の個人データ（メールアドレス等）はホワイトリストを含むデータから削除されるため、システムへのログインは不可能となる。
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{マスター管理}: イベント作成、競技種目設定、ホワイトリスト管理、クラス人数設定
