...
加えて、クラスの所属を表す \texttt{\{class\_name\}\_rep} ロールや、クラス・競技ごとの割り当てを表す動的ロールを補助的に用いる。
...
Roleの制限について、各ユーザーは上記のベースロールに加えて補助ロールを併用できる。\\
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{マイページ}: プロフィール設定、所属・参加情報確認
        \item \textbf{クラス情報}: \texttt{\{class\_name\}\_rep} ロール保有者向け(クラスのメンバー全員)のクラス人数・出席状況・勝ち進み状況の確認
