...
\begin{itemize}
    ...
    \item \textbf{Events}: 球技大会等のイベント単位。アクティブなイベントは別テーブル（active\_event）で管理。同年度の切り替えでは同名クラスへプロフィールを引き継ぎ、年度変更時は卒業処理とプロフィール再設定を行う。
...
\subsection{データベース設計}
詳細なデータベース設計は以下の通りである。マイグレーション0020適用時点で、データベースは全40テーブルおよび1ビューで構成される。
ここでは主要なテーブルを記載する。
