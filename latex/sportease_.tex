...
SportEase は、学校行事におけるスポーツ大会運営を支援するための Web システムである。
主な利用者は一般学生、行事委員、システム管理者であり、競技情報の閲覧、試合結果の登録、クラス・チーム管理、通知配信、昼企画管理等を一元的に扱う。

本書では、まずシステム全体の目的と構成を示した上で、個人情報の取り扱い、外部設計、内部設計、データベース設計、API設計の順に説明する。

\subsection{システム全体像}
本システムは Docker コンテナ上で動作し、Traefik を入口として、SvelteKit 製のフロントエンド、Go + Gin 製のバックエンド、MySQL データベースで構成される。
ユーザーはブラウザから HTTPS でアクセスし、フロントエンド経由でAPIやリアルタイム更新機能を利用する。
...
本システムでは、ユーザー認証にGoogle OAuth2を採用している。学校のGoogle アカウント(@sendai-nct.jp, @sendai-nct.ac.jp)のドメインのみログインを許容するよう、メールアドレスのドメインを検証して制御している。
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 卒業生のメールアドレスは学校ドメインから変更されるため、ドメイン検証によりシステムへのログインは不可能となる。
    \end{itemize}
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        クラス・学年 & ユーザーがどのクラス・学年に所属しているかを示す \\
        \bottomrule
    \end{tabularx}
\end{table}
...
クラス単位の操作対象は、初回プロフィール設定で保存したユーザーの所属クラス（\texttt{class\_id}）から決定する。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l| X|}
        ...
        Admin (行事委員) & イベント全体の管理、試合結果の入力・承認、スケジュールの変更、MIC投票の管理など、運営に必要な全機能にアクセス可能。 \\ \midrule
        Root (システム管理者) & システム設定、マスターデータ（スポーツ、クラス等）の管理を行う最高権限。 \\
        \bottomrule
    \end{tabularx}
\end{table}

所属クラスはプロフィール完了後、一般ユーザーが変更できない。\\
補助ロールはクラス管理、競技参加管理、通知のロールフィルタリング等に利用する。
競技参加ロールはイベントIDを持ち、大会ごとに独立して付与・解除する。基本ロールと大会共通の任意ロールはイベントIDを持たない。
...
\begin{itemize}
    ...
    \begin{itemize}
        \item 開催中のイベント情報の閲覧
        \item マイページでの参加競技、今後の試合、直近の試合結果の集約表示
        \item マイページでのクラス出席状況、競技進捗、最新通知の集約表示
        \item 大会要項PDF、競技要項PDF、公開中アンケートへのアクセス
        \item 各競技のトーナメント表・対戦組み合わせの閲覧
        \item 通常競技・盤上競技の試合結果および勝敗の確認
        \item 結果一覧で完了した通常試合と公開済み昼競技結果を横断表示
    ...
    \begin{itemize}
        ...
        \item Rootへの通知申請および申請メッセージの送受信
        \item MyIDによる参加確認
        \item ヘッダーからのバグ報告・非公開脆弱性報告・新機能提案
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 昼企画（リレー・綱引き・借り物競争等）の予選・決勝結果の入力
        \item 競技タイピング結果JSONの検証・取込・置換
        \item 盤上競技の順位・得点入力
    \end{itemize}
    \item \textbf{運営管理}:
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 複数昼企画セッションのグループ分け・テンプレート管理
        \item 盤上競技を除く通常競技トーナメントの一括自動生成・プレビュー
        \item 固定16クラスの将棋A・B、オセロのシード順指定・専用生成
        \item 保存済みトーナメントのExcel出力、クラス別結果のCSV・PDF出力
        \item データベースおよびアップロード済みファイルのバックアップ出力
    \end{itemize}
    \item \textbf{全体通知・申請管理}:
    \begin{itemize}
        \item 全ユーザー、特定ロールまたは個人ユーザーに向けた任意のお知らせ通知の配信
        \item 学生からの通知申請の承認・却下
        \item 大会要項PDFの管理
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{ダッシュボード (ホーム)}: ログイン後の初期画面。ユーザーロールに応じたメニューを表示し、端末のローカルストレージにショートカット表示設定を保存。
    \item \textbf{学生向けメニュー (Student)}:
    \begin{itemize}
        \item \textbf{マイページ}: プロフィール、クラス成績、参加試合・結果、クラス状況、最新通知、大会資料・アンケートの確認
        \item \textbf{クラス情報}: プロフィールで設定した自クラスの人数・出席状況・勝ち進み状況の確認
        \item \textbf{競技・イベント情報}: 競技詳細、タイムテーブル、通常・盤上トーナメント、結果一覧、点数一覧の閲覧
        \item \textbf{昼企画}: 昼競技結果の確認
        \item \textbf{その他}: MyIDチェックイン、通知確認、通知申請
    \end{itemize}
    \item \textbf{運営管理者メニュー (Admin)}:
    \begin{itemize}
        \item \textbf{試合管理}: 試合結果入力、昼競技結果入力
        \item \textbf{参加管理}: クラス・チーム編成管理、MyIDバーコード読み取り、試合別・クラス別チェックイン状況確認、出席登録
        \item \textbf{運営設定}: 統計ダッシュボード、ロール管理、競技詳細設定、MIC投票
    \end{itemize}
    \item \textbf{システム管理者メニュー (Root)}:
    \begin{itemize}
        \item \textbf{マスター管理}: イベント作成、競技種目設定、クラス人数設定
        \item \textbf{全体設定}: 雨天モード設定、トーナメント生成・管理、昼競技管理
        \item \textbf{コンテンツ管理}: 通知管理、通知申請管理、大会要項アップロード、MIC確認
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{XSS対策}: SvelteKitのオートエスケープ機能により、ユーザー入力のスクリプト実行を防止している。
    \item \textbf{API認可}: 各エンドポイントにおいてミドルウェア（Middleware）による認証・認可チェックを行い、権限のないリソースへのアクセスを遮断している。ヘルスチェックとOAuth開始・コールバックを除き、クラス一覧やWebSocket接続を含む大会データAPIは認証を要求する。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l l}
        ...
        カテゴリ & 技術 \\
        \midrule
        フロントエンド & SvelteKit, TailwindCSS, JavaScript \\
        バックエンド & Go 1.26, Gin, Gorilla WebSocket \\
        データベース & MySQL 8.0 \\
...
\begin{itemize}
    \item \textbf{Users / Roles}: ユーザーID、メールアドレス、表示名、およびベースロール・大会単位の競技ロールを管理。
    \item \textbf{Events}: 球技大会等のイベント単位。アクティブなイベントは別テーブル（active\_event）で管理。同年度の春秋では同名クラスへプロフィールとクラス在籍人数を引き継ぎ、年度変更時は卒業処理とプロフィール再設定を行う。専教は在籍人数・アンケート得点の対象外として扱う。
    ...
    \item \textbf{Board Games}: 固定16クラスによる将棋A・B、オセロのシード、出場者、順位、得点を管理。
    \item \textbf{Noon Games}: 昼企画のセッション、試合、結果、ポイント、テンプレート実行を管理。
    \item \textbf{Notifications}: Web Push通知の購読情報、ロール/個人宛て通知履歴、および通知リクエストのワークフローを管理。
\end{itemize}

\subsection{データベース設計}
詳細なデータベース設計は以下の通りである。マイグレーション0023適用時点で、データベースは全41テーブルおよび1ビューで構成される。
ここでは主要なテーブルを記載する。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        notification\_filters & JSON & 通知フィルター設定 (default: ["general"]) \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        \midrule
        id & INT & Primary Key \\
        name & VARCHAR(50) & ロール名（初期値は root, admin, student。動的ロールも格納） \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        event\_id & INT & 大会単位ロールのイベントID (FK, NULLなら大会共通) \\
        event\_scope\_id & INT & NULLを0として一意性判定する仮想列 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        survey\_url & VARCHAR(500) & アンケートURL (NULLable) \\
        is\_survey\_published & BOOLEAN & アンケート通知済みフラグ \\
        status & ENUM & イベント状態 (preparing, upcoming, active, archived) \\
        hide\_scores & BOOLEAN & 得点非表示フラグ \\
        duplicate\_registration\_threshold & INT & 2競技登録を許可するクラス人数上限 \\
        is\_mic\_voting\_enabled & BOOLEAN & MIC投票有効フラグ \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        location & VARCHAR(255) & 任意の開催場所 \\
        template\_key & VARCHAR(64) & 盤上/昼競技テンプレート識別子 \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        sport\_id & INT & 競技ID (FK) \\
        entry\_key & VARCHAR(64) & 同一競技内の代表枠識別子（例: A, B, SUBSTITUTE） \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        winner\_team\_id & INT & 確定した勝者チームID (FK) \\
        next\_match\_id & INT & 次戦ID (FK) \\
        status & VARCHAR(50) & 試合状態 (scheduled, in\_progress, completed) \\
        match\_start\_time & TIMESTAMP & 開始予定日時 \\
        match\_end\_time & TIMESTAMP & 終了日時 \\
...
\begin{table}[H]
    \centering
    \caption{round\_check\_ins (通常競技チェックイン)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        event\_id / sport\_id / match\_id & INT & イベント・競技・試合ID (FK) \\
        round & INT & チェックイン対象ラウンド \\
        user\_id / team\_id & CHAR(36)/INT & チェックインしたユーザー・所属チームID (FK) \\
...
\begin{table}[H]
    \centering
    \caption{class\_scores (クラス得点集計ビュー ※VIEWのため実テーブルではない)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        reason & VARCHAR(64) & 理由（制約付きの定数値） \\
        source\_match\_id & INT & 対象試合ID (FK) \\
        noon\_game\_session\_id & INT & 対象昼競技セッションID (FK) \\
        noon\_game\_match\_id & INT & 対象昼競技試合ID (FK) \\
        board\_game\_run\_id & INT & 対象盤上競技実行ID (FK) \\
        created\_at & TIMESTAMP & 作成日時 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\subsubsection{Board Game Tables}
将棋・オセロの専用トーナメントに関わるテーブル群。通常競技の一括生成とは分離して管理する。

\begin{table}[H]
    \centering
    \caption{board\_game\_runs (盤上競技実行)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        event\_id / sport\_id & INT & イベント・競技ID (FK) \\
        game\_type & ENUM & 競技種別 (shogi, othello) \\
        name / description / location & VARCHAR/TEXT & 名称・説明・会場 \\
        rules\_pdf\_url / scheduled\_date & VARCHAR/DATE & 要項PDF・実施日 \\
        win\_points / rank\_points & INT/JSON & 1勝点・順位点 \\
        regular\_minutes / final\_minutes & INT & 通常戦・決勝の試合時間 \\
        players\_per\_class / substitutes\_per\_class & INT & クラスごとの選手・補欠人数 \\
        status & ENUM & 状態 (draft, published, completed) \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{board\_game\_entries / board\_game\_entry\_members}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        run\_id / tournament\_id / team\_id / class\_id & INT & 実行・ブラケット・チーム・クラスID (FK) \\
        slot\_key & VARCHAR(64) & ブラケット枠 (A, B, MAIN) \\
        seed\_number & INT & ドラッグ＆ドロップで決めたシード順 \\
        entry\_id / user\_id & INT/CHAR(36) & エントリーと選手ID (FK) \\
        member\_order / is\_substitute & INT/BOOLEAN & 選手順・補欠フラグ \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{board\_game\_rankings (盤上競技順位)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        run\_id / tournament\_id / entry\_id & INT & 実行・ブラケット・エントリーID (FK) \\
        rank\_number / win\_count & INT & 順位・勝数 \\
        win\_points / rank\_points / total\_points & INT & 勝数点・順位点・合計点 \\
        recorded\_by / recorded\_at & CHAR(36)/TIMESTAMP & 記録者・記録日時 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        id & INT & Primary Key \\
        event\_id & INT & イベントID (FK) \\
        template\_key & VARCHAR(64) & 競技テンプレート識別子 \\
        name & VARCHAR(255) & 企画名 \\
        description & TEXT & 説明 \\
        scheduled\_at & DATETIME & 実施日時 \\
        ...
        status & ENUM & 状態 (draft, finalized, published) \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        scheduled\_at & DATETIME & 予定日時 \\
        location & VARCHAR(255) & 開催場所 \\
        format & VARCHAR(100) & 試合形式 \\
...
\begin{table}[H]
    \centering
    \caption{noon\_game\_check\_ins (昼競技チェックイン)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        event\_id / session\_id / match\_id & INT & イベント・セッション・試合ID (FK) \\
        user\_id & CHAR(36) & チェックインしたユーザーID (FK) \\
        checked\_in\_at & TIMESTAMP & チェックイン日時 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{noon\_game\_template\_run\_matches (テンプレート実行試合対応)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        run\_id & INT & テンプレート実行ID (FK) \\
        match\_id & INT & 試合ID (FK, Unique) \\
        match\_key & VARCHAR(50) & 試合キー識別子 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{noon\_game\_points (昼企画ポイント)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        session\_id & INT & セッションID (FK) \\
        match\_id & INT & 試合ID (FK, NULLable) \\
        class\_id & INT & クラスID (FK) \\
        points & INT & ポイント数 \\
        reason & VARCHAR(255) & 理由 \\
        source & ENUM & 付与元 (result, manual, typing\_system) \\
        created\_by & CHAR(36) & 作成者ID (FK) \\
        created\_at & TIMESTAMP & 作成日時 \\
        \bottomrule
    \end{tabularx}
...
\begin{table}[H]
    \centering
    \caption{noon\_game\_match\_entries (昼企画試合エントリー)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        match\_id & INT & 試合ID (FK) \\
        entry\_index & INT & エントリー順序 \\
        side\_type & ENUM & 種別 (class, group) \\
        class\_id & INT & クラスID (FK, NULLable) \\
        group\_id & INT & グループID (FK, NULLable) \\
        display\_name & VARCHAR(255) & 表示名 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{noon\_game\_result\_details (昼企画結果詳細)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        result\_id & INT & 結果ID (FK) \\
        entry\_id & INT & エントリーID (FK) \\
        placement\_rank & INT & 順位 (NULLable) \\
        competition\_score & INT & 競技内スコア (NULLable) \\
        points & INT & 付与ポイント \\
        note & VARCHAR(255) & メモ \\
        entry\_resolved\_name & VARCHAR(255) & エントリー解決済み名前 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{noon\_game\_template\_default\_groups (テンプレートデフォルトグループ)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        template\_key & VARCHAR(50) & テンプレートキー (year\_relay, course\_relay, tug\_of\_war 等) \\
        group\_index & INT & グループ順序 \\
        group\_name & VARCHAR(255) & グループ名 \\
        class\_names & JSON & クラス名リスト \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{noon\_game\_typing\_system\_imports (タイピング結果取込履歴)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id / session\_id & INT & 取込ID・昼競技セッションID (FK) \\
        export\_id / sha256 & CHAR(36/64) & 外部出力ID・ファイルハッシュ \\
        status / action & ENUM & 成否と操作種別 (import, replace) \\
        replaced\_export\_id & CHAR(36) & 置換対象の外部出力ID \\
        requested\_by / requested\_at & CHAR(36)/TIMESTAMP & 実行者・実行日時 \\
        filename / payload\_size & VARCHAR/INT & ファイル名・サイズ \\
        results & JSON & 検証済み取込結果 \\
        is\_active & BOOLEAN & 現在有効な取込フラグ \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        title & TEXT & タイトル \\
        body & TEXT & 本文 \\
        type & VARCHAR(50) & 通知種別 (general, match\_my\_class, finals, all\_matches) \\
...
\begin{table}[H]
    \centering
    \caption{notification\_request\_messages (通知リクエストメッセージ)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Column & Type & Description \\
        \midrule
        id & INT & Primary Key \\
        request\_id & INT & 通知リクエストID (FK) \\
        sender\_id & CHAR(36) & 送信者ID (FK) \\
...
\subsection{API設計}
RESTful APIのエンドポイント一覧（主要なもの）。ここでは代表的なものを記載する。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        GET & /api/classes & クラス一覧取得（student/admin/root） \\
        GET & /api/scores/class & クラス得点情報取得 \\
        GET & /api/barcode/teams & 自分の所属チーム一覧取得 \\
        POST & /api/barcode/check-in & MyIDバーコード読み取りによる対象試合チェックイン（通常競技・昼競技、参加本登録を兼ねる、admin/root） \\
        GET & /api/barcode/matches/:match\_id/check-ins & 選択試合のクラス別チェックイン人数・表示名・未チェックイン者取得（admin/root） \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        GET & .../events/:id/board-game-runs & 盤上競技ブラケット・出場者・順位取得 \\
        GET & .../events/:id/noon-game/sessions & 公開済み昼企画セッション一覧取得 \\
        GET & .../events/:id/noon-game/sessions/:id & 昼企画セッション詳細取得 \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        Method & Path & Description \\
        \midrule
        GET & /api/ws/tournaments/:id & トーナメントリアルタイム更新（認証必須） \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        PUT & .../board-game-runs/:id/tournaments/:id/rankings & 盤上競技順位・得点確定 \\
        POST & .../noon-game/sessions/:id/typing-system/import & タイピング結果JSON取込 \\
...
\begin{table}[H]
    \centering
    \caption{Admin MIC API (/api/admin/mic)}
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \toprule
        Method & Path & Description \\
        \midrule
        GET & .../eligible-classes & MIC投票対象クラス取得 \\
        POST & .../vote & MIC投票 \\
        GET & .../votes & MIC投票一覧 \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.1cm}|X|p{4.3cm}|}
        \toprule
        Method & Path & Description \\
        \midrule
        ...
        POST & .../events/:id/tournaments/generate-all & 通常競技トーナメント一括生成（盤上競技を除く） \\
        POST & .../events/:id/tournaments/generate-preview & 一括生成プレビュー \\
        GET & .../events/:id/tournament-templates/board-game/classes & 盤上競技対象16クラス取得 \\
        POST & .../events/:id/tournament-templates/board-game/run & 盤上競技・シード順生成 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{Root API (/api/root、続き)}
    \begin{tabularx}{\textwidth}{|p{3.1cm}|X|p{4.3cm}|}
        \toprule
        Method & Path & Description \\
        \midrule
        GET/POST & .../events/:id/noon-game/sessions & 昼競技セッション一覧・作成 \\
        PUT/DELETE & .../events/:id/noon-game/sessions/:id & 昼競技セッション更新・削除 \\
        POST & .../events/:id/noon-game/templates/.../run & 各昼競技テンプレート実行 \\
        POST & .../noon-game/sessions/:id/typing-system/import & タイピング結果JSON取込 \\
        GET & .../events/:id/tournaments/export/excel & 保存済みトーナメントExcel出力 \\
        GET & .../db/export & データベースSQLダンプ出力 \\
        GET & .../uploads/export & アップロードファイルZIP出力 \\
