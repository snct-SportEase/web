...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{競技・イベント情報}: 競技詳細、タイムテーブル、トーナメント、点数一覧の閲覧
...
\begin{itemize}
    \item \textbf{Users / Roles}: ユーザーID、メールアドレス、表示名、およびベースロール・動的ロールの割り当てを管理。
...
\subsection{データベース設計}
詳細なデータベース設計は以下の通りである。データベースは全34テーブルおよび1ビューで構成される。
ここでは主要なテーブルを記載する。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        status & ENUM & イベント状態 (upcoming, active, archived) \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        POST & /api/barcode/check-in & MyIDバーコード読み取りによる参加本登録・ラウンドチェックイン \\
        \bottomrule
    \end{tabularx}
\end{table}
