...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item MyIDバーコードの読み取りおよび参加本登録の確認
        \item MIC投票および状況確認
        \item ロールの付与・剥奪
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{その他}: MyID参加確認、通知確認、通知申請
    \end{itemize}
    \item \textbf{運営管理者メニュー (Admin)}:
    \begin{itemize}
        \item \textbf{試合管理}: 試合結果入力、昼競技結果入力
        \item \textbf{参加管理}: クラス・チーム編成管理、MyIDバーコード読み取り、参加本登録確認、出席登録
...
\subsection{データベース設計}
詳細なデータベース設計は以下の通りである。マイグレーション0022適用時点で、データベースは全40テーブルおよび1ビューで構成される。
ここでは主要なテーブルを記載する。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        POST & /api/barcode/check-in & MyIDバーコード読み取りによる参加本登録・ラウンドチェックイン（admin/root） \\
        \bottomrule
    \end{tabularx}
\end{table}
