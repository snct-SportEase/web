...
- ダッシュボードのショートカット表示を端末ごとにカスタマイズ
- クラス別の参加状況・スコア・進行状況の可視化
- Web Push通知の購読管理とPWA対応
- ヘッダーからのバグ報告・非公開の脆弱性報告・新機能提案

### Root（システム管理者）
- 大会（イベント）の作成・更新・ステータス管理（準備中・予定・開催中・アーカイブ）
- 通常競技のトーナメント一括生成・プレビューと、盤上競技（将棋・オセロ）の専用トーナメント生成
- ドラッグ＆ドロップによる盤上競技のシード順設定
- 複数の昼競技セッション、タイピング結果JSON取込、ノーンゲーム設定管理
- 保存済みトーナメントのExcel出力、クラス別結果のCSV・PDF出力
- データベースとアップロード済みファイルのバックアップ出力
- 競技の登録、チーム一覧の参照
- クラス在籍人数の更新（CSVインポート、同年度の春大会から秋大会への引き継ぎに対応。専教は対象外）
- 通知の作成と、ロール単位または個人単位の配信先管理
...
### Admin（大会運営担当）
- クラス・チーム編成とメンバー割当
- 将棋のA・B代表／補欠、オセロ代表の登録
- 審判ロールなど任意ロールの付与・削除
- 競技詳細情報や競技要項PDFのアップロード
- 出席登録とクラス別出席状況の参照
- 試合開始時刻・進行ステータスの更新、開催中大会の試合結果入力
- 開催中大会のノーンゲーム試合結果登録、MIC投票
- MyIDバーコード読み取りによる参加本登録・通常競技／昼競技の試合別チェックイン、クラス別の対象人数・未チェックイン者確認
...
### Student（参加者）
- マイページでの参加競技・今後の試合・直近の試合結果・クラス成績の確認
- クラス全体の出席・勝ち進み状況と最新のお知らせの確認
- マイページから大会要項・競技要項PDF、公開中アンケートへのアクセス
- 結果一覧で通常競技の完了試合と公開済み昼競技結果を横断確認
- 通常競技と盤上競技のトーナメント表・勝敗結果の閲覧
- 通常競技・昼競技のタイムテーブルと、春季・秋季の順位・試合結果を含む過去大会アーカイブの閲覧
...
- バックエンド: Go 1.26 + Gin, MySQL, Redis, WebSocket, WebPush
...
- npm
- Go 1.26 系（`backapp/go.mod` の `toolchain` 指定に従う）
...
| `TRUSTED_PROXY_CIDRS` | バックエンドへ `X-Forwarded-For` を渡せるリバースプロキシのIP/CIDR（カンマ区切り）。単体起動時の既定値は `127.0.0.1/32,::1/128`、Docker Compose の既定値は `172.16.0.0/12` |
...
独自タイピングシステムは単体で動作し、6チームの確定得点と順位をJSONでSportEaseへ取り込みます。仕様は、[共通仕様の範囲](./docs/typing_system_core_scope.md)、[確定結果JSON仕様](./docs/typing_system_results_json_spec.md)、[参加チーム構成JSON](./docs/typing-team-structure-v1.json)、[共通入力適合テストJSON](./docs/typing-input-tests-v1.json)、[本戦・予備問題プリセットJSON](./docs/typing-problem-presets-v1.json)、[運営・試合状況機能仕様](./docs/typing_system_operations_spec.md)、[スコア計算仕様](./docs/typing_system_scoring_spec.md)、[入力判定仕様](./docs/typing_system_input_spec.md)、[問題・問題セット仕様](./docs/typing_system_problem_spec.md)を参照してください。

#### 認証・プロキシの運用上の注意

- Googleログインでは、OAuthの`state`と`nonce`を一回限りのHttpOnly Cookieで検証し、Google IDトークンの署名・発行者・対象クライアント・メール確認状態を検証します。Google Cloud Consoleには、実際に公開する`GOOGLE_REDIRECT_URL`を完全一致で登録してください。
- セッションは24時間で失効します。ログイン時に`session_token`と同時にCSRFトークンも発行され、Cookieを伴う更新系APIはCSRF検証が必要です。通常のブラウザ操作ではSvelteKitプロキシが同一オリジンを確認してヘッダーを付加するため、追加操作は不要です。
- `TRUSTED_PROXY_CIDRS`には、バックエンドの直前にあるプロキシのネットワークだけを設定してください。広すぎるCIDRやインターネット全体を指定すると、クライアントIPを偽装され、IPベースのレート制限が信頼できなくなります。Compose構成ではTraefikの転送ヘッダー信頼を無効化し、SvelteKitが正規化した値だけをバックエンドへ渡します。

#### Push通知の運用上の注意

- Push購読先はHTTPS・ポート443・許可済みPushサービスのホストだけを受け付けます。`WEBPUSH_ALLOWED_HOSTS`を設定する場合は、利用するサービスだけをカンマ区切りで指定してください（例: `fcm.googleapis.com,*.push.apple.com`）。
- 1ユーザーは最大5端末を登録できます。同じ購読先を別ユーザーへ移管することはできません。購読登録はユーザーごとに1時間10回までです。
- 通知送信時、失効または不正な購読情報は自動削除されます。通知本文そのものはアプリ内通知として保存されるため、Pushを無効にした利用者も通知一覧で確認できます。
