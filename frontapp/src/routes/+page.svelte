    ...
    <div class="max-w-md w-full bg-white shadow-md rounded-lg p-6 sm:p-8">
        ...
        {#if user}
            <div class="text-center">
                <p class="text-gray-600 mb-4">ログイン中: {user.email}</p>
                <form 
                    action="/api/logout" 
                    method="POST" 
                    use:enhance={() => {
                ...
                >
                    <button 
                        type="submit"
                        class="w-full bg-red-500 text-white py-2 px-4 rounded-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition duration-200"
                    >
                        サインアウト
                    </button>
                </form>
