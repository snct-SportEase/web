...
export async function load({ fetch, request, url }) {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/student/notification-requests`, { headers });
    if (res.ok) {
    ...
    console.error('Failed to load notification requests:', error);
  ...
  if (targetId) {
    try {
      const detailRes = await fetch(`${BACKEND_URL}/api/student/notification-requests/${targetId}`, { headers });
      if (detailRes.ok) {
