...
通知を利用できるユーザーには、ヘッダーとサイドバーに端末の状態を示すラベルが表示されます。

- `PWA未設定`: 現在PWAのstandaloneモードで起動していません。ラベルをクリックし、対応ブラウザでは「デバイスにインストール」、それ以外ではOS別の手順からPWAを導入してください。
- `通知未設定`: 現在の端末に有効なPush購読がありません。ダッシュボードの「通知を有効にする」を実行してください。
- `通知拒否中`: ブラウザで通知権限が拒否されています。サイト設定から通知を許可してから再度有効化してください。

PWAをインストール済みでも通常のブラウザタブから開いている場合は、ブラウザの制約により `PWA未設定` と判定されることがあります。ホーム画面またはアプリ一覧のSportEaseアイコンから起動して確認してください。
