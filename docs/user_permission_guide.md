...
| **クラス所属** | 初回ログイン時に自動付与されるロール。自クラスのチーム管理が可能 | 全学生 |

### 権限の取得方法
- **Root/Admin/Student**: ホワイトリストにメールアドレスを登録する際に設定
- **クラス所属**: 初回ログイン時にクラスを選択した際に自動付与（`クラス名_rep` 形式）
...
**注意**: MIC投票はAdmin/Rootのみが行えます。投票対象は1〜2年生のみです。
