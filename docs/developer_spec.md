...
- MyIDバーコードのチェックイン状況は試合ごとに返却します。通常競技は対戦チームの割当済みメンバー、昼競技は試合対象クラスのうち当該昼競技へ割当済みのメンバーだけを母数にします。昼競技のチェックイン記録は `noon_game_check_ins` に保存します。
- 参加本登録確認の専用画面は廃止し、バーコード読み取り画面でクラス別の対象人数・チェックイン人数・表示名・未チェックイン者を確認します。
- モバイルでは共通ヘッダーの報告・提案導線をオーバーフローメニューへ集約し、メニュー外のタップで閉じます。PWA未設定はダッシュボード上のカードでも案内します。
