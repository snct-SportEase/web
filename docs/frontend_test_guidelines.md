...
| 機能 | 概要 | コンポーネント | E2E | 備考 |
| :--- | :--- | :---: | :---: | :--- |
| 管理者ダッシュボード閲覧 | 統計情報のリアルタイム確認 | ○ | ○ | `manage-dashboard.svelte.spec.js` / `admin-manage-dashboard.spec.js` で確認済み |
| チームメンバー割り当て | 競技参加メンバーの登録・削除 | ○ | ○ | `class-management.svelte.spec.js` / `admin-class-management.spec.js` で確認済み |
| ロール付与・削除 | 審判ロール等の付与 | ☓ | ☓ | |
