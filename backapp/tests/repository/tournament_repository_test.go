...
func TestTournamentRepository_UpdateMatchResult(t *testing.T) {
	t.Run("Success - Update match and advance winner", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta("SELECT id, tournament_id, round, match_number_in_round, team1_id, team2_id, COALESCE(winner_team_id, CASE WHEN team1_score > team2_score THEN team1_id WHEN team2_score > team1_score THEN team2_id END) AS winner_team_id, status, next_match_id, match_start_time, is_bronze_match, is_loser_bracket_match, loser_bracket_round, loser_bracket_block, rainy_mode_start_time FROM matches WHERE id = ? FOR UPDATE")).
...
func TestTournamentRepository_UpdateMatchResultRejectsInvalidOrFinishedMatch(t *testing.T) {
	const matchQuery = "SELECT id, tournament_id, round, match_number_in_round, team1_id, team2_id, COALESCE(winner_team_id, CASE WHEN team1_score > team2_score THEN team1_id WHEN team2_score > team1_score THEN team2_id END) AS winner_team_id, status, next_match_id, match_start_time, is_bronze_match, is_loser_bracket_match, loser_bracket_round, loser_bracket_block, rainy_mode_start_time FROM matches WHERE id = ? FOR UPDATE"
	columns := []string{"id", "tournament_id", "round", "match_number_in_round", "team1_id", "team2_id", "winner_team_id", "status", "next_match_id", "start_time", "is_bronze_match", "is_loser_bracket_match", "loser_bracket_round", "loser_bracket_block", "rainy_mode_start_time"}

	t.Run("finished match is rejected after row lock", func(t *testing.T) {
		db, mock, err := sqlmock.New()
		require.NoError(t, err)
		defer db.Close()
		...
		mock.ExpectBegin()
		mock.ExpectQuery(regexp.QuoteMeta(matchQuery)).WithArgs(42).
			WillReturnRows(sqlmock.NewRows(columns).AddRow(42, 3, 0, 0, 10, 20, 10, "finished", nil, "", false, false, nil, nil, nil))
		mock.ExpectRollback()

		err = repo.UpdateMatchResult(42, 2, 1, 10)
		assert.ErrorIs(t, err, repository.ErrMatchResultAlreadyEntered)
		assert.NoError(t, mock.ExpectationsWereMet())
	})

	t.Run("tie winner outside participating teams is rejected", func(t *testing.T) {
		db, mock, err := sqlmock.New()
		require.NoError(t, err)
		defer db.Close()
		repo := repository.NewTournamentRepository(db)
		...
		mock.ExpectBegin()
		mock.ExpectQuery(regexp.QuoteMeta(matchQuery)).WithArgs(42).
			WillReturnRows(sqlmock.NewRows(columns).AddRow(42, 3, 0, 0, 10, 20, nil, "pending", nil, "", false, false, nil, nil, nil))
		mock.ExpectRollback()

		err = repo.UpdateMatchResult(42, 1, 1, 999)
		assert.ErrorIs(t, err, repository.ErrInvalidMatchResult)
		assert.NoError(t, mock.ExpectationsWereMet())
	})
}
