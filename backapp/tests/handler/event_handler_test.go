...
func TestEventHandler_CreateEvent(t *testing.T) {
	...
	t.Run("Success - Create Spring Event", func(t *testing.T) {
		...
		mockEventRepo.On("CreateEventWithClasses", mock.AnythingOfType("*models.Event"), models.DefaultClassNames()).Return(int64(1), nil).Once()
	...
	t.Run("Success - Create Preparing Event", func(t *testing.T) {
		...
		repo.On("CreateEventWithClasses", mock.MatchedBy(func(event *models.Event) bool {
			return event.Status == models.EventStatusPreparing
		}), models.DefaultClassNames()).Return(int64(1), nil).Once()
	...
	t.Run("Success - Create Autumn Event with Spring Event existing", func(t *testing.T) {
		...
		mockEventRepo.On("CreateEventWithClasses", mock.AnythingOfType("*models.Event"), models.DefaultClassNames()).Return(int64(2), nil).Once()
	...
	t.Run("Success - Create Autumn Event without Spring Event", func(t *testing.T) {
		...
		mockEventRepo.On("CreateEventWithClasses", mock.AnythingOfType("*models.Event"), models.DefaultClassNames()).Return(int64(3), nil).Once()
	...
	t.Run("Returns Error When Default Class Creation Fails", func(t *testing.T) {
		...
		eventRepo.On("CreateEventWithClasses", mock.AnythingOfType("*models.Event"), models.DefaultClassNames()).Return(int64(0), errors.New("db error")).Once()
...
func TestEventHandler_NotifySurvey(t *testing.T) {
	...
	t.Run("Success - Notify Survey", func(t *testing.T) {
		...
		h := handler.NewEventHandler(mockEventRepo, nil, nil, mockNotificationRepo, mockUserRepo, "dummyPublicKey", "dummyPrivateKey")
		...
		mockEventRepo.On("GetEventByID", eventID).Return(event, nil).Once()
		mockNotificationRepo.On("CreateNotification", "大会アンケートのお願い", "大会に関するアンケート機能が公開されました。「2025春季スポーツ大会」についてダッシュボードの一番上のリンクからアンケートにご協力ください。", "general", "test-user-id", &eventID).Return(int64(10), nil).Once()
	...
	t.Run("Error - Event has no survey URL configured", func(t *testing.T) {
		...
		h := handler.NewEventHandler(mockEventRepo, nil, nil, mockNotificationRepo, mockUserRepo, "", "")
